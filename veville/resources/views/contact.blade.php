...
<form method="post" action="" class="group-form">
    ...
    <input id="username" name="name" type="text" class="w-100 form-control" value="@if(Auth::check()){{ Auth::user()->name }}@endif"><br>
    <label for="emailUser">Votre e-mail: </label>
    <input id="emailUser" name="email" type="text" class="w-100 form-control" value="@if(Auth::check()){{ Auth::user()->email }}@endif"><br>
